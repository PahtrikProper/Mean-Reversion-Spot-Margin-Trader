...
from ..utils.helpers import now_ms, interval_minutes, taker_fee_for, maker_fee_for, supported_intervals, fetch_borrow_rate_safe
...
from ..trading.bybit_client import BybitPrivateClient, fetch_last_klines, get_instrument_info
...
class LiveRealTrader:
    ...
    def __init__(
        ...
        try:
            self.client.ensure_trading_setup(self.symbol, leverage=float(exit_params.leverage))
        ...
        self.mark_price = float(df_last_seed["close"].iloc[-1])
    ...
    def _execute_entry(self, close_price: float, ts_utc: str, bar_ts: pd.Timestamp):
        ...
        try:
            ...
            order_id = self.client.place_market_order(self.symbol, "Buy", qty)
            if self.last_signal:
    ...
    def _execute_exit(self, close_price: float, reason: str, ts_utc: str):
        ...
        try:
            ...
            order_id = self.client.place_market_order(self.symbol, "Sell", qty_to_close)
            if self.last_signal:
                self.last_signal["placed"] = True
            self._refresh_state()
            summary = self.client.get_execution_summary(self.symbol, order_id)
    ...
    def _run_reoptimise(self):
        ...
        try:
            ...
            # Fetch live borrow rate once before the re-opt loop
            _live_borrow_rate = fetch_borrow_rate_safe(self.client)

            for sym in SYMBOLS:
                try:
                    inst = get_instrument_info(sym)
                except Exception as e:
                    log.warning(f"[REOPT] {sym}: get_instrument_info failed — skipping: {e}")
                    continue

                for iv in supported_intervals(CANDLE_INTERVALS):
                    try:
                        df_last = download_seed_history(sym, DAYS_BACK_SEED, iv)
            ...
            if new_mc_score > 0:
                ...
                try:
                    self.client.ensure_trading_setup(self.symbol, leverage=float(self.leverage))
...
def download_seed_history(symbol: str, days_back: int, interval: str):
    """Download last-price kline history for seeding the live trader."""
    import engine.utils.db_logger as _db
    end_ts   = now_ms()
    start_ts = end_ts - int(days_back * 24 * 60 * 60 * 1000)

    log.info(f"Downloading {interval}m history for {symbol} ({days_back} days)...")
    df_last = fetch_last_klines(symbol, interval, start_ts, end_ts)
    log.info(f"Candles downloaded: {len(df_last)}")
    ...
    log.info(f"Seeded {n_last} candles → DB")
